...
private val B7_ZH = mapOf(
    ...
    // B8 自动备份（每天首次启动写一份 JSON，只留最近 3 份）
    "data.autoBackup" to "🗄 自动备份",
    "data.autoBackup.pick" to "选择备份文件夹",
    "data.autoBackup.change" to "更换文件夹",
    "data.autoBackup.noFolder" to "尚未选择备份文件夹 —— 自动备份未生效",
    "data.autoBackup.folder" to "文件夹：{name}",
    "data.autoBackup.daily" to "每天首次启动时自动备份",
    "data.autoBackup.desc" to "备份写入你选择的文件夹，只保留最近 {n} 份；不联网、不上传",
    "data.autoBackup.count" to "已保存 {n} 份",
    "data.autoBackup.last" to "上次备份：{t}",
    "data.autoBackup.never" to "还没有备份过",
    "data.autoBackup.now" to "立即备份一次",
    "data.autoBackup.done" to "已备份：{name}",
    "data.autoBackup.pruned" to "已清理旧备份 {n} 份",
    "data.autoBackup.failed" to "备份失败：{msg}",

    // A5 错题本排序 + 复习筛选；B6 重做；B9 查重
    "notebook.sort" to "⇅ 排序",
    "notebook.sort.newest" to "最近添加",
    "notebook.sort.oldest" to "最早添加",
    "notebook.sort.next" to "下次复习时间",
    "notebook.sort.most" to "复习次数",
    "notebook.sort.weak" to "最不熟优先",
    "review.filter.allSubjects" to "全部科目",
    "review.filter.onlyWeak" to "只看不熟",
    "review.filter.clear" to "清除筛选",
    "review.empty.filtered" to "当前筛选下没有要复习的题",
    "redo.entry" to "✏️ 手写重做",
    "redo.hint.shoot" to "拍下你在纸上写的解答（这一张会交给 AI 批改）",
    "redo.next" to "下一题",
    ...
    "export.failed" to "导出失败，请重试",
    "settings.fontSize" to "字号",
    "settings.fontSize.desc" to "只影响 App 内的文字大小（界面与对话区同步），不动系统设置",
    "settings.fontSize.small" to "小",
    "settings.fontSize.normal" to "标准",
    "settings.fontSize.large" to "大",
    "settings.fontSize.xlarge" to "特大",
    "notebook.titlePlain" to "错题本",
    "notebook.listCount" to "当前列表共 {n} 题",
...
private val B7_EN = mapOf(
    ...
    // B8 auto backup (one JSON on the first launch of each day, only the last 3 kept)
    "data.autoBackup" to "🗄 Auto backup",
    "data.autoBackup.pick" to "Choose backup folder",
    "data.autoBackup.change" to "Change folder",
    "data.autoBackup.noFolder" to "No backup folder chosen yet — auto backup is inactive",
    "data.autoBackup.folder" to "Folder: {name}",
    "data.autoBackup.daily" to "Back up on the first launch of each day",
    "data.autoBackup.desc" to "Written into the folder you choose, only the last {n} kept; no network, no upload",
    "data.autoBackup.count" to "{n} backup(s) kept",
    "data.autoBackup.last" to "Last backup: {t}",
    "data.autoBackup.never" to "No backup yet",
    "data.autoBackup.now" to "Back up now",
    "data.autoBackup.done" to "Backed up: {name}",
    "data.autoBackup.pruned" to "{n} old backup(s) removed",
    "data.autoBackup.failed" to "Backup failed: {msg}",

    // A5 notebook sorting + review filter; B6 redo; B9 duplicate check
    "notebook.sort" to "⇅ Sort",
    "notebook.sort.newest" to "Recently added",
    "notebook.sort.oldest" to "Oldest first",
    "notebook.sort.next" to "Next review time",
    "notebook.sort.most" to "Most reviewed",
    "notebook.sort.weak" to "Least mastered first",
    "review.filter.allSubjects" to "All subjects",
    "review.filter.onlyWeak" to "Weak ones only",
    "review.filter.clear" to "Clear filters",
    "review.empty.filtered" to "Nothing to review under the current filters",
    "redo.entry" to "✏️ Redo by hand",
    "redo.hint.shoot" to "Photograph your handwritten solution (this one goes to the AI for grading)",
    ...
    "export.failed" to "Export failed, please try again",
    ...
    "settings.fontSize.normal" to "Default",
    "settings.fontSize.large" to "Large",
    "settings.fontSize.xlarge" to "Extra large",
    "notebook.titlePlain" to "Notebook",
    "notebook.listCount" to "{n} problem(s) in the current list",
...
private val B7_JA = mapOf(
    ...
    // B8 自動バックアップ（毎日最初の起動時に 1 件、最新 3 件だけ保持）
    "data.autoBackup" to "🗄 自動バックアップ",
    "data.autoBackup.pick" to "バックアップ先フォルダを選ぶ",
    "data.autoBackup.change" to "フォルダを変更",
    "data.autoBackup.noFolder" to "バックアップ先が未設定 —— 自動バックアップは動作しません",
    "data.autoBackup.folder" to "フォルダ：{name}",
    "data.autoBackup.daily" to "毎日最初の起動時に自動バックアップ",
    "data.autoBackup.desc" to "選んだフォルダに書き出し、最新 {n} 件だけ保持します。通信も送信もしません",
    "data.autoBackup.count" to "{n} 件を保存中",
    "data.autoBackup.last" to "前回のバックアップ：{t}",
    "data.autoBackup.never" to "まだバックアップしていません",
    "data.autoBackup.now" to "今すぐバックアップ",
    "data.autoBackup.done" to "バックアップしました：{name}",
    "data.autoBackup.pruned" to "古いバックアップを {n} 件削除しました",
    "data.autoBackup.failed" to "バックアップに失敗：{msg}",

    // A5 並べ替え・復習フィルタ；B6 やり直し；B9 重複チェック
    "notebook.sort" to "⇅ 並べ替え",
    "notebook.sort.newest" to "新しい順",
    "notebook.sort.oldest" to "古い順",
    "notebook.sort.next" to "次回復習が近い順",
    "notebook.sort.most" to "復習回数の多い順",
    "notebook.sort.weak" to "苦手な順",
    "review.filter.allSubjects" to "すべての科目",
    "review.filter.onlyWeak" to "苦手だけ",
    "review.filter.clear" to "フィルタ解除",
    "review.empty.filtered" to "現在のフィルタでは復習する問題がありません",
    "redo.entry" to "✏️ 手書きでやり直す",
    "redo.hint.shoot" to "紙に書いた解答を撮影してください（この 1 枚を AI が添削します）",
    "redo.next" to "次の問題",
    ...
    "export.failed" to "書き出しに失敗しました。もう一度お試しください",
    "settings.fontSize" to "文字サイズ",
    "settings.fontSize.desc" to "アプリ内の文字だけを変更します（画面と会話欄が連動）。端末の設定は変更しません",
    "settings.fontSize.small" to "小",
    "settings.fontSize.normal" to "標準",
    "settings.fontSize.large" to "大",
    "settings.fontSize.xlarge" to "特大",
    "notebook.titlePlain" to "誤答ノート",
    "notebook.listCount" to "現在のリストは {n} 件",
...
private val B7_KO = mapOf(
    ...
    // B8 자동 백업(매일 처음 실행할 때 1개, 최근 3개만 보관)
    "data.autoBackup" to "🗄 자동 백업",
    "data.autoBackup.pick" to "백업 폴더 선택",
    "data.autoBackup.change" to "폴더 변경",
    "data.autoBackup.noFolder" to "백업 폴더가 아직 없습니다 — 자동 백업이 동작하지 않습니다",
    "data.autoBackup.folder" to "폴더: {name}",
    "data.autoBackup.daily" to "매일 처음 실행할 때 자동 백업",
    "data.autoBackup.desc" to "선택한 폴더에 저장하며 최근 {n}개만 보관합니다. 네트워크·업로드 없음",
    "data.autoBackup.count" to "{n}개 보관 중",
    "data.autoBackup.last" to "마지막 백업: {t}",
    "data.autoBackup.never" to "아직 백업한 적이 없습니다",
    "data.autoBackup.now" to "지금 백업",
    "data.autoBackup.done" to "백업 완료: {name}",
    "data.autoBackup.pruned" to "오래된 백업 {n}개를 삭제했습니다",
    "data.autoBackup.failed" to "백업 실패: {msg}",

    // A5 정렬·복습 필터, B6 다시 풀기, B9 중복 확인
    "notebook.sort" to "⇅ 정렬",
    "notebook.sort.newest" to "최근 추가순",
    "notebook.sort.oldest" to "오래된 순",
    "notebook.sort.next" to "다음 복습이 빠른 순",
    "notebook.sort.most" to "복습 횟수 많은 순",
    "notebook.sort.weak" to "취약한 순",
    "review.filter.allSubjects" to "모든 과목",
    "review.filter.onlyWeak" to "취약한 것만",
    "review.filter.clear" to "필터 해제",
    "review.empty.filtered" to "현재 필터에 해당하는 복습 문제가 없습니다",
    "redo.entry" to "✏️ 손으로 다시 풀기",
    "redo.hint.shoot" to "종이에 쓴 풀이를 촬영하세요 (이 사진을 AI가 채점합니다)",
    "redo.next" to "다음 문제",
    ...
    "export.failed" to "내보내기에 실패했습니다. 다시 시도해 주세요",
    ...
    "settings.fontSize.normal" to "기본",
    "settings.fontSize.large" to "크게",
    "settings.fontSize.xlarge" to "아주 크게",
    "notebook.titlePlain" to "오답노트",
    "notebook.listCount" to "현재 목록 {n}개",
